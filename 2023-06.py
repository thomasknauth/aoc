...
# faster if their button was held longer, but time spent holding the button counts against the total
# race time. You can only hold the button at the start of the race, and boats don't move until the
# button is released.
...
# millimeters. You could hold the button anywhere from 14 to 71516 milliseconds and beat the record,
...
def part1(f):
    times, dists = map(lambda s: map(int, s.strip().split(':')[1].split()), f.readlines())

    wins = 1
    for time, dist in zip(times, dists):
        wins *= len(list(filter(lambda x: x > dist, y(time))))
...
# Use binary search to find the value at which f(x) > dist. To be able to use the built-in bisect
# module, we construct a simple class that implements __getitem__ ([] operator) and __len__.
def part2():
    # There is only a single input, so we hard-code it.
    time = 44806572
    dist = 208158110501102

    func = LazyF(time)
    i = bisect.bisect_left(func, dist, 0, time//2)
    return time - i * 2 + 1

if __name__ == '__main__':

    # print(part1(io.StringIO(test_input)))
    print(part1(io.StringIO(p1_input)))
    print(part2())
