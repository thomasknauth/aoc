...
def part2(f):
    ...
    for (idx, line) in enumerate(f.readlines(), start=1):
        ...
        # Make sure we count each card at least once.
        if not idx in cards:
            cards[idx] = 1

        for i in range(len(overlap)):
            cards[i+idx+1] += cards[idx]

    return sum([cards[i] for i in range(1, max_card+1)])
