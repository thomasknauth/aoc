...
def part1(fn):
    ...
    for (idx, line) in enumerate(open(fn).readlines(), start=1):
        valid_game = True

        line = line.removeprefix(f'Game {idx}:')
        rounds = line.split(';')

        for round in rounds:
            for count, color in map(lambda s: s.split(), round.split(',')):
...
def part2(fn):
    ...
    for (idx, line) in enumerate(open(fn).readlines(), start=1):
        ...
        min_count = {'blue': 0, 'green': 0, 'red': 0}

        for round in rounds:
            for count, color in map(lambda s: s.split(), round.split(',')):
                if int(count) > min_count[color]:
                    min_count[color] = int(count)
        
        sum += min_count['green'] * min_count['red'] * min_count['blue']
